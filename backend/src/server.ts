...
server.post<{ Body: Entry }>("/create/", async (req, reply) => {
  ...
  newEntryBody.created_at = newEntryBody.created_at
    ? new Date(req.body.created_at)
    : new Date();
    newEntryBody.scheduled_date = req.body.scheduled_date
...
server.put<{ Params: { id: string }; Body: Entry }>(
  ...
  async (req, reply) => {
    ...
    updatedEntryBody.created_at = updatedEntryBody.created_at
      ? new Date(req.body.created_at)
      : new Date();
      updatedEntryBody.scheduled_date = req.body.scheduled_date
      ? new Date(req.body.scheduled_date)
      : null;
